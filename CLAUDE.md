...
| `src/core/rule_induction.py` | **경계 규약을 전문에서 층계로 찾는다**(D-116·D-117): 1단 목차(대조 80%↑면 멈춤, 목차 쪽은 세지 않음) → 2단 시각(되풀이 기호 `symbols`·내려쓰기 단독 `indent_alone`) → 3단 텍스트(날짜·짧은 행 끝 어휘·행머리 글자) → 4단 LLM(`extract_start_patterns_llm` — 표본만 보내 정해진 종류로 답받고 코드가 세어 확인) → 사람. 점수는 «횟수×간격 고름×날짜 사슬×(줄지어 오면 감점)», «쪽마다 한 번 같은 자리»(판심)는 버린다. 결과는 `segmentation_rules` 데이터 — 코드에 책 이름을 넣지 않는다. 편성 탭 「경계 제안」이 답 한 줄(`stage`)로 보이고, 규칙이 빈 문헌의 자동 트리는 먼저 이것을 돌려 저장한다. 네 책 눈가림 재현(천진담초 36/36·36/42) |
