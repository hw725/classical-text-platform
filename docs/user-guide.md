...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 이 책의 확정본에서 되풀이되는 표지를 스스로 찾아
   ...
   - 답이 틀렸을 때만 **「자세히 · 고치기」**를 펼치세요. 신호마다 횟수·보기가 체크박스로 뜹니다(판심·엽수처럼
     ...
     **「전부 적용해 새로 세우기」**로 그 설정대로 개요를 세웁니다. 어휘를 직접 더할 수도 있고(「어휘 더하기」 —
     행 첫머리 어휘는 `^有`처럼 `^`를 앞에, 기호 한 글자는 그대로), 모르겠으면 **「해제·본문에서 뽑기」**(LLM)가
     후보를 목록에 넣어 줍니다.
   - 되풀이되는 표지를 하나도 못 찾은 책(«찾지 못함»)은 답 줄에 **「LLM에 시작 표지 묻기」** 단추가 나옵니다.
     짧은 행 표본만 모델에 보내 «시작 행의 공통점»을 정해진 종류(행머리 어휘·행끝 어휘·기호·없음)로 답받고,
     전문에서 넷 이상 되풀이되는 것만 목록에 넣습니다. 그래도 없으면 「＋ 경계 넣기」로 찍는 책입니다.
   - 해제(解題)가 있으면 신호 목록 아래 「참고·억제」의 **해제·참고 텍스트**에 통째로 붙여 넣으세요
     (2만 자여도 됩니다). 해제는 LLM 스위치·단추만 읽습니다.
   - LLM을 쓰는 곳은 셋뿐입니다 — 목차 줄의 **「LLM으로 구조화」** 스위치(목차 쪽 텍스트만 보냅니다),
     「해제·본문에서 뽑기」, 「LLM에 시작 표지 묻기」. 표시가 없는 것은 모델을 부르지 않습니다. 사이드바
     「자동 트리」도 그렇습니다 — 언제나 규칙만 돕니다. 구조화 스위치를 켠 채로 개요를 세우려면 편성 인덱스의
