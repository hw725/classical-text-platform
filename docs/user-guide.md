...
○권점·표제 어휘)를 스스로 찾아 **묶음 > 기사 > 조각** 개요를 세웁니다 — 무엇으로 세웠는지 알림에 적힙니다.
규칙만 쓰고 LLM은 부르지 않습니다. 기사를 누르면 원본 이미지의 그 자리에 **시작 행이 점선**으로 표시되고,
...
1. 사이드바 **「내용」 ▸ 「자동 트리」**. 이 책의 확정본에서 되풀이되는 표지를 스스로 찾아
   ...
   - 찾는 순서는 사람이 책을 보는 순서입니다 — **목차 → 눈에 띄는 것(되풀이되는 기호·내려쓰기) → 텍스트
     패턴 → LLM → 사람**. 위에서 확실하면 거기서 멈추고, 목차가 규약인 책은 목차 쪽의 짧은 행을 본문 규약으로
     배우지 않습니다. 텍스트 패턴은 미리 정해 둔 종류가 아니라 «행 첫머리·행 끝·기호 뒤에 어떤 글이 되풀이되는가»를
     책마다 새로 셉니다 — 날짜·談草·卷之一·「一、」 같은 꼴이 거기서 나옵니다. 행마다 같은 글이 되풀이되는
     것(판권·두주)은 표제로 보지 않습니다.
   - 편성 인덱스 **「경계 제안」**을 누르면 **답 한 줄**이 먼저 보입니다 — «이 책의 규약: 시각 신호: 기호
     「○」 217회 — 2단». 그 아래 후보 목록과 단추 셋(닫기·골라 적용·전부 적용해 새로 세우기)뿐입니다.
   - 전에 저장한 설정이 이번 답과 다르면 답 줄에 **「권고대로」** 단추가 나옵니다 — 누르면 체크가 이번 권고로
     돌아가고, 「후보 보기」나 「전부 적용해 새로 세우기」를 눌러야 저장됩니다.
   - 답이 틀렸을 때만 **「자세히 · 고치기」**를 펼치세요. 신호마다 횟수·보기가 체크박스로 뜹니다(판심·엽수처럼
     쪽마다 같은 자리에 오는 것은 뺀 채로). 체크를 바꾸고 **「후보 보기」**로 후보를 다시 보거나,
     **「전부 적용해 새로 세우기」**로 그 설정대로 개요를 세웁니다. 어휘를 직접 더할 수도 있고(「어휘 더하기」 —
     행 첫머리 어휘는 `^有`처럼 `^`를 앞에, 기호 한 글자는 그대로), 모르겠으면 「LLM에 묻기」의 «해제·본문에서
     표제 어휘 뽑기»가 후보를 목록에 넣어 줍니다.
   - 모델을 부르는 단추는 답 줄의 **「LLM에 묻기」** 하나뿐입니다. 누르면 창에서 셋 중 고릅니다.
     **목차 항목 구조화**(목차 쪽 텍스트와 해제만 보냄 — 켜 두면 문헌 설정에 저장되어 「후보 보기」·「전부 적용」
     때마다 씀), **해제·본문에서 표제 어휘 뽑기**(해제 + 짧은 행 표본 ≤60줄), **시작 표지 묻기**(짧은 행·행갈음
     뒤·내려쓴 행 표본 ≤80줄을 보내 «시작 행의 공통점»을 행머리 어휘·행끝 어휘·기호·없음 중 하나로 받고, 전문에서
     넷 이상 되풀이되는 것만 목록에 넣음 — 표본은 «시작 자리 80줄 / +앞뒤 한 행 / 6쪽 통째 / 권 전체» 중 고르고,
     고르면 «N줄 · M자»가 바로 보임). 되풀이되는 표지를 못 찾은 책(«찾지 못함»)이면 «시작 표지 묻기»가
     기본으로 켜져 있습니다. 그래도 없으면 「＋ 경계 넣기」로 찍는 책입니다.
   - 해제(解題)가 있으면 신호 목록 아래 「참고·억제」의 **해제·참고 텍스트**에 통째로 붙여 넣으세요
     (2만 자여도 됩니다). 해제는 「LLM에 묻기」만 읽습니다.
   - 편성에서 모델을 부르는 것은 **「LLM에 묻기」** 하나뿐이고, 거기서 고른 것만 보냅니다. 그 밖의 단추와 사이드바
     「자동 트리」는 언제나 규칙만 돕니다. «목차 항목 구조화»를 켜 둔 채로 개요를 세우려면 편성 인덱스의
     「전부 적용해 새로 세우기」를 누르세요.
