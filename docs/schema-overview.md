...
| segmentation_rules | object? | 편성 규칙(D-088·D-116·D-117) — 신호 스위치 `signals`, `title_words`·`head_words`, 시각 신호 `symbols`·`indent_alone`, 꼴 `head_templates`·`tail_templates`(접은 글자 N·G·Z, D-119), 판심 `furniture`, `suppress`, `reference_text`, `toc_llm`, `origin`(빈 값=아직 없음·induced·manual). 프로그램이 전문에서 층계로 찾아 채우고 사람이 고친다 |
