...
1. **층계.** 사람이 책을 펼쳤을 때 보는 순서로 세고, 위 단이 확실하면 아래 단은 보조로만 남긴다.
   ...
   - 4단 **LLM** — 통계가 아무것도 못 찾았을 때 사람이 누르면. 표본 행(짧은 행·행갈음 뒤·내려쓴 행 ≤ 80줄)만
     보내고 답은 **정해진 종류**(행머리 어휘·행끝 어휘·기호·없음)로만 받는다. 모델이 경계를 찍지 않는다 — «규칙
...
3. **편성 탭은 답 한 줄.** 「경계 제안」을 누르면 «이 책의 규약: 시각 신호: 기호 「○」 217회 — 2단» 한 줄과 후보
   ...
   참고·억제는 «자세히 · 고치기»에 접혀 있고, 층계가 잘못 골랐을 때만 펼친다. 0단이면 «자세히»가 열리고
   「LLM에 시작 표지 묻기」 단추가 답 줄에 나온다. 저장된 설정이 이번 권고와 다르면(예: D-116 때 저장한 뒤
