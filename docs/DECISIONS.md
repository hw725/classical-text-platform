...
## D-119: 신호 «종류»를 코드에서 없앤다 — 발견기 하나: 어느 자리에 어떤 문자열이 편중되어 되풀이되는가

**날짜:** 2026-09-07
**상태:** 확정
**관련:** D-116(규약 도출), D-117(층계), 네 책 눈가림 실측

### 무엇이 있었나

D-116·D-117의 도출기는 «신호 종류»를 코드가 정했다 — 행머리 날짜·○+날짜·卷頭·행 첫머리 1자·짧은 행 끝 2자·
기호·내려쓰기 단독·판식 셋, 열한 가지. 값(有·談草·○)은 책에서 왔지만 «행 첫머리 1자를 본다»는 결정은 제가
세 책을 보고 고른 것이었다. 사용자가 세 번 물었다: «이거 다 하드코딩 규칙이냐», «11종류가 특정 문헌에 과적합돼
보인다», 그리고 «내가 말했는데 안 하고 넘어가는 건 시한폭탄이다» — D-117에서 «종류를 없애고 발견기로»를 제안해
놓고 층계만 넣고 종류를 8 → 11로 늘렸기 때문이다.

### 결정

1. **코드에 남는 것은 셋.** ① 판식 물리(짧은 행·행갈음·내려쓰기·bbox), ② 발견기 하나 — «자리 × 되풀이
   문자열», ③ 층계 순서(목차 → 시각 → 텍스트 → LLM → 사람). 그 밖의 «종류»는 없다.
2. **발견기.** 자리는 셋: 행 첫머리(head)·행 끝(tail)·기호 뒤(after). 문자열은 «접은 글자» 1~4자. 접기는
   언어 지식이지 책의 규칙이 아니다 — 한자 수사 → N, 天干 → G, 地支 → Z, 그 밖은 그대로. 그래서 「二十八日」
   「十日」은 같은 꼴 «N日»이 되고 「一、」「二、」는 «N、»가 된다. 가족(자리, 꼴)마다 나온 행을 모아
   횟수(4↑)·**편중**(그 자리 횟수 ÷ 기대 횟수 = 전문 빈도 × 행 수 ÷ 글자 수, 3배↑)·간격의 고름·밀도·연속·
   판심(쪽마다 같은 자리)으로 고른다. 긴 꼴이 짧은 꼴의 80%를 설명하면 짧은 꼴을 버리고(「有詩」가 「有」를),
   같은 자리에서 행이 80% 겹치는 두 꼴은 점수 높은 쪽만 남긴다.
3. **문법은 «찾은 뒤의 판정»에만.** 찾은 꼴의 보기 행 중 80% 이상이 날짜 문법이면 `signals.date`, 기호 뒤가
   날짜면 `signals.mark`, 행 끝이 卷 이름이면 `signals.volume`으로 배정하고 같은 스위치로 판정된 꼴들은 하나로
   합친다(「翌日」「初N日」「同日」 → «행 첫머리 날짜» 90회). 접은 글자가 든 꼴은 템플릿(`head_templates`·
   `tail_templates`), 아니면 어휘(`head_words`·`title_words`), 기호는 `symbols`. 저장 형식은 D-116·D-117 그대로.
4. **표제는 행마다 다르다.** 가족의 행들이(접은 글로) 절반 이상 같은 글이면 표제 규약이 아니라 판권·두주·되풀이
   표기다 — 天津談草의 「同」 54행(판권 문구 「同十一日」 13행 + 「同」 25행)이 그렇게 걸러졌다. 이 판정 없이는
   두주 후보가 날짜 사슬을 흐려 재현이 34/36으로 떨어졌다. 날짜뿐인 행 끝 꼴(«N日» 한 줄짜리 두주)도 표제
   규약이 아니다.
5. **3단 선택은 1위 점수의 60% 이상, 최대 여섯.** 40%로 두면 문집에서 31가족이 켜졌다.
6. 날짜 문법에 「同十一日」(같은 달의 11일)을 더했다 — 同은 «같은»이라는 말이지 책의 규약이 아니다.

### 실측 (눈가림 — 코드에 책 이름·글자 없음)

| 책 | 멈춘 단 | 발견기가 찾은 것 | 제안 |
|---|---|---|---|
| 浩齋辰巳日錄 65쪽본 | 2단 | 기호 「○」 217 · 「◯」 158 · 기호 뒤 날짜 325 · 행 첫머리 날짜 65 | 후보 615 · 채택 583 |
| 天津談草 | 3단 | 행 첫머리 날짜 67(是月N日·N月初N·N日海関·同N日) · 행 끝 「談草」 26. 「同」 54는 되풀이로 제외 | 재현 **36/36** · 정밀 **36/42** (손 규칙과 같음) |
| 雲養集 1책 | 3단(목차 9/102 약함) | 행 첫머리 날짜 90 · 행 끝 「共賦」 36 「韻」 24 「也」 25 · 행 첫머리 「右」 32 「同」 30 | 후보 305 · 채택 183 |

D-117의 열한 종류가 세던 것이 전부 발견기의 값으로 다시 나왔고, 종류 이름은 코드에 없다.

### 한계

- 발견기는 «되풀이되는 표면 문자열»만 본다. 시집처럼 제목이 그냥 짧은 행인 책은 여전히 4단(LLM)·찍기다.
- 「也」처럼 문장 종결이 행 끝에 몰리는 문집에서는 종결 조사가 규약처럼 보일 수 있다. 정답 색인이 없는 운양집은
  판단 보류 — 사용자가 «자세히 · 고치기»에서 끄면 된다.
- 접기 표(수사·干支)는 한문 고서의 언어 지식이다. 다른 문자 체계(한글·가나)의 책은 접기 표를 더해야 한다.

### 보정 (2026-09-08, Codex gpt-6-astra 교차검증)

Codex는 크레딧이 두 번 떨어져 수정은 못 했지만 결함 주장을 회귀 테스트 16개로 남겼다. 코드와 대조해 13건을
받고 3건은 설계 근거로 거절했다. 네 책 재현은 전후가 같다(36/36·36/42, 305/183, 615/583).

받은 것:
- **글자 목록이 하나 남아 있었다.** 행 끝 꼴을 버릴 때 `_DATE_FOLD_CHARS`(N月日初是同…)로 갈랐다 — 이 결정의
  §1을 어긴 것이다(「和而不同」의 「同」이 날짜 글자라는 이유로 버려졌다). 이제 «가족 행의 80% 이상이 행 전체가
  날짜 문법인가»로 판정한다. 날짜 문법은 `parse_date_head` 하나다.
- 판정 표본은 가족 **전체**다(앞 40행만 보면 책 앞쪽이 날짜, 뒤쪽이 어휘인 가족을 날짜로 잘못 판정한다).
  기호 뒤 가족은 한 행에 기호가 여럿일 때 **그 꼴이 붙은 자리만** 본다(「●又本文●初一日」의 「●又」가 뒤쪽 날짜로
  mark가 되지 않게).
- 발견기·제안기 모두 **접은 뒤에 자른다** — 원문 8·12자를 먼저 자르면 긴 수사(「一千二百三十四、」)가 «N»으로
  접혀 뒤의 「、」를 못 본다.
- `rules_from_signals`는 이번 셈에 안 나온 저장값(손으로 넣은 어휘)을 **보존**하고, 목록에 나왔는데 고르지 않은
  값만 뺀다. `rules_are_empty`는 템플릿만 있는 규칙도 «정한 것»으로 본다(안 그러면 자동 트리가 덮어쓴다).
- `verify_pattern`(LLM 후보 확인)은 발견기와 같은 `_solid`로 권고를 정한다 — 전에는 연속 비율을 안 봐서
  「又山」「又川」처럼 줄지어 오는 덩어리가 권고됐다. 「□」가 든 값은 거른다.
- 정렬에 id 타이브레이크(발견기 dict 순서에 결과가 달리지 않게), 편중은 전문에 없는 꼴이면 0, 행이 넷 미만이어도
  목차가 결정적이면 1단.

거절한 것(테스트는 «정한 행동»을 고정하게 고쳐 남겼다):
- **문법 가족에도 판심 필터를 적용하라** — 판심은 «쪽마다 한 번 같은 자리에 같은 글»이다. 날짜는 행마다 다른
  글이라, 한 쪽에 한 기사인 일기를 판심으로 버리게 된다. 밀도 문턱(`_solid`)은 문법 가족에도 그대로 적용된다.
- **월만 있는 날짜(「三月春」)도 되풀이 판정에 세라** — 같은 달의 시제를 판권의 «같은 날짜 되풀이»로 오판한다.
- **mark를 끄면 앞 열 끝 ○ 뒤의 행 첫머리 날짜도 사라져야 한다** — 행 첫머리에 온 날짜는 date 스위치의 것이다.
  mark를 끄면 ○ 보너스만 잃는다. 제안기 주석이 반대로 적혀 있어 고쳤다.
